...
def main():
 ...
 #   img_name = "Images/IBETGroup17.jpg"
    ...
    all_rgbs = []
    columns = ('pixel_r','pixel_g','pixel_b','local_r','local_g','local_b','edges_r','edges_g','edges_b','label')
...
def add_label(evt,label):
    global all_rgbs,columns
    if(len(all_rgbs)==9):
        all_rgbs.append(label)
        print(all_rgbs)

        df = pd.DataFrame(data=all_rgbs, index=columns).T
        print(df)
        # if file does not exist write header
        if not os.path.isfile('data/Tar-Print-Data-wFall-woutWater-Testing.csv'):
            df.to_csv('data/Tar-Print-Data-wFall-woutWater-Testing.csv', header='columns')
        else:  # else it exists so append without writing the header
            df.to_csv('data/Tar-Print-Data-wFall-woutWater-Testing.csv', mode='a', header=False)
...
def build_interface():
    ...
    btn_perv.bind('<Button-1>', lambda event: add_label(event, 'norm_pervious'))

    btn_fall_perv = tk.Button(root, text='Fall Pervious')
    btn_fall_perv.pack(side=tk.LEFT)
    btn_fall_perv.bind('<Button-1>', lambda event: add_label(event, 'fall_pervious'))

    btn_dry_perv = tk.Button(root, text='Dry Pervious')
    btn_dry_perv.pack(side=tk.LEFT)
    btn_dry_perv.bind('<Button-1>', lambda event: add_label(event, 'dry_pervious'))

    btn_shade_perv = tk.Button(root, text='Shaded Pervious')
    btn_shade_perv.pack(side=tk.LEFT)
    btn_shade_perv.bind('<Button-1>', lambda event: add_label(event, 'shaded_pervious'))

    btn_imperv = tk.Button(root, text='Impervious')
    btn_imperv.pack(side=tk.LEFT)
    btn_imperv.bind('<Button-1>', lambda event: add_label(event, 'norm_impervious'))

    btn_tan_imperv = tk.Button(root, text='Tan pervious')
    btn_tan_imperv.pack(side=tk.LEFT)
    btn_tan_imperv.bind('<Button-1>', lambda event: add_label(event, 'tan_impervious'))

    btn_water = tk.Button(root, text='Water')
    btn_water.pack(side=tk.LEFT)
    btn_water.bind('<Button-1>', lambda event: add_label(event, 'water'))
